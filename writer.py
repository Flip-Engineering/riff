...
def openrouter_text(payload, messages):
    import urllib.request
    request = urllib.request.Request("https://openrouter.ai/api/v1/chat/completions",
        data=json.dumps({"model": payload["model"], "messages": messages, "max_tokens": payload.get("writer_tokens", 768)}).encode(),
        headers={"Authorization": "Bearer " + payload["api_key"], "Content-Type": "application/json", "X-Title": "Riff"})
    with urllib.request.urlopen(request, timeout=300) as response:
        result = json.load(response)
    if result["choices"][0].get("finish_reason") == "length":
        raise ValueError("The writing budget ended before the draft was complete. Increase it and try again.")
    return result["choices"][0]["message"]["content"].replace(payload["api_key"], "[redacted]")
...
def edit_score(payload):
    ...
    text = openrouter_text(payload, [{"role": "system", "content": system}, {"role": "user", "content": json.dumps(content)}])
    try:
        ...
        raise ValueError("The composer did not return a complete score. Try again or increase the writing budget.") from None
...
def write(payload, raw_path=None):
    ...
    try:
        ...
        raise ValueError("The writer did not finish a readable draft. Try again or raise the writing token budget.")
    def words(value):
