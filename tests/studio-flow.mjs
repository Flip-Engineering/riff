...
try {
  ...
  await page.locator("#sound-color").fill("0");
  assert.notEqual(await page.locator("#sound-view-canvas").evaluate(canvas => canvas.toDataURL()), originalSurface);
  await page.locator("#sound-color").fill("0.7");
  const textured = await page.locator("#sound-view-canvas").evaluate(canvas => canvas.toDataURL());
  await page.locator("#sound-texture").fill("0");
  assert.notEqual(await page.locator("#sound-view-canvas").evaluate(canvas => canvas.toDataURL()), textured);
  ...
  assert.deepEqual(await page.evaluate(() => JSON.parse(localStorage.getItem("riff.soundAppearance"))), { surface: 0, motion: 0, color: .7, texture: 0 });
  assert.deepEqual(await page.evaluate(() => formRecipe()), draft);
  await page.locator("#sound-surface").fill("0.72");
  await page.locator("#sound-motion").fill("1");
  await page.locator("#sound-texture").fill("0.48");
  await page.locator(".sound-appearance > summary").click();
  ...
  console.log("PASS Surface, movement, color and texture are independently adjustable, persist, and leave the musical draft unchanged");
