...
const RiffArtwork = (() => {
  ...
  function scene(seed, seconds = 0, motion = []) {
    ...
    for (let ring = 0; ring < rings; ring++) {
      ...
      for (let point = 0; point < steps; point++) {
        ...
        const radius = (100 + f * 60.75 + 22 * Math.sin(3 * t + phase) * f) * (1 + bass * .095) + strain;
        const x = Math.cos(t) * radius;
        const y = (Math.sin(t) * (68 + f * 35.1) + 30 * Math.sin(2 * t + phase) * f) * (1 + bass * .065);
        const wave = middle * 10 * Math.sin(3 * t + phase - time * 1.8 + f * 4)
          + air * 3.2 * Math.sin(15 * t - time * 5.5 + f * 8)
          + attack * 13 * Math.sin(6 * t - time * 3.8 + f * 13)
          + crest * 1.4 * Math.sin(27 * t - time * 6 + f * 15);
        const z = -22 + f * 40 + fold * (22 + 30 * Math.sin(2 * t + phase))
          + 18 * Math.sin(3 * t + phase) * f + (.28 + fold * .72) * wave + arrival * 10 * fold;
  ...
  function createSurfaceRenderer() {
    ...
    const fragment = compile(gl.FRAGMENT_SHADER, `#version 300 es
      ...
      void main() {
        ...
        float band=surfaceUV.y*112.0;
        float ridge=1.0-smoothstep(0.10,0.10+fwidth(band),abs(fract(band)-0.5));
        material=mix(material,mix(ink,paper,light*0.67),ridge*0.29);
        pixel=vec4(material,1.0);
      }
    `);
  ...
  function draw(context, width, height, seed, seconds = 0, motion = []) {
    ...
    if (!drawSurface(context, g, scale)) for (const face of g.faces) {
      ...
      context.fillStyle = g.shades[face.shade]; context.fill();
      // Fill shared raster edges; the separate contour is the fine physical ridge.
      context.strokeStyle = g.shades[face.shade]; context.lineWidth = .3; context.stroke();
  ...
  return { draw, svg, scene };
})();

function drawSeedArtwork(context, width, height, seed, seconds = 0, motion = []) {
  RiffArtwork.draw(context, width, height, seed, seconds, motion);
}
