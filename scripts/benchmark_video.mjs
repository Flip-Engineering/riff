...
const profile = process.env.RIFF_BENCH_PROFILE || "master";
assert(["share", "master"].includes(profile));
...
try {
  ...
  const environment = { platform: platform(), release: release(), cpu: cpus()[0].model,
    browser: browser.version(), profile, source_audio_sha256: sourceAudioHash, load_average_start: loadavg(),
  ...
  for (const [width, height, fps] of [[1920, 1080, 30], [2160, 2160, 30], [2160, 2160, 60]]) {
    for (const duration of durations) {
      ...
      for (const transport of transports) {
        ...
        await page.locator("#video-profile").selectOption(profile);
        ...
        assert.equal(job.profile, profile);
        ...
        const result = { name, width, height, fps, duration, transport, profile, original_audio_sha256: sourceAudioHash, wall_seconds: elapsed,
